...
public class DireccionHome {
	...
	public Direccion findById(java.lang.Integer id) {
		...
		try {
			Direccion instance = (Direccion) sessionFactory.getCurrentSession().get("Direccion", id);
			if (instance == null) {
	...
	public List findByExample(Direccion instance) {
		...
		try {
			List results = sessionFactory.getCurrentSession().createCriteria("Direccion").add(Example.create(instance))
					.list();
