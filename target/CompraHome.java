...
/**
 * Home object for domain model class Compra.
 * @see .Compra
...
public class CompraHome {
	...
	public void persist(Compra transientInstance) {
	...
	public Compra merge(Compra detachedInstance) {
		...
		try {
			Compra result = (Compra) sessionFactory.getCurrentSession().merge(detachedInstance);
	...
	public Compra findById(java.lang.Integer id) {
		...
		try {
			Compra instance = (Compra) sessionFactory.getCurrentSession().get("Compra", id);
			if (instance == null) {
	...
	public List findByExample(Compra instance) {
		...
		try {
			List results = sessionFactory.getCurrentSession().createCriteria("Compra").add(Example.create(instance))
					.list();
