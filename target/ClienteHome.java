...
public class ClienteHome {
	...
	public Cliente findById(java.lang.Integer id) {
		...
		try {
			Cliente instance = (Cliente) sessionFactory.getCurrentSession().get("Cliente", id);
			if (instance == null) {
	...
	public List findByExample(Cliente instance) {
		...
		try {
			List results = sessionFactory.getCurrentSession().createCriteria("Cliente").add(Example.create(instance))
					.list();
